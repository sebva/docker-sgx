...
RUN wget --progress=dot:mega -O - https://github.com/01org/linux-sgx/archive/sgx_2.1.tar.gz | tar -xz && \
    cd linux-sgx-sgx_2.1 && \
    patch -p1 -i ../install-psw.patch && \
    ./download_prebuilt.sh 2> /dev/null && \
    make -s -j$(nproc) sdk_install_pkg psw_install_pkg && \
    ./linux/installer/bin/sgx_linux_x64_sdk_2.1.42002.bin --prefix=/opt/intel && \
    ./linux/installer/bin/sgx_linux_x64_psw_2.1.42002.bin && \
    cd .. && rm -rf linux-sgx-sgx_2.1
